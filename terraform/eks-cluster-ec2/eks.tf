...
resource "aws_iam_role" "kaledros-eks-cluster" {
...
resource "aws_iam_role_policy_attachment" "kaledros-eks-cluster-policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.kaledros-eks-cluster.name
}
...
resource "aws_eks_cluster" "kaledros-eks-cluster" {
  name     = var.cluster_name
  version  = var.cluster_version
  role_arn = aws_iam_role.kaledros-eks-cluster.arn

  vpc_config {
    security_group_ids      = [aws_security_group.kaledros-eks-cluster.id]
  ...
  depends_on = [aws_iam_role_policy_attachment.kaledros-eks-cluster-policy]
}
